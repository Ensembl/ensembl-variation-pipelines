...
def main(metadata_file, urn):
    ...
    # Match the API metadata format used by downstream steps.
    if selected_entry:
        creator = first_present(
            selected_entry.get("createdBy"),
            (selected_experiment or {}).get("createdBy"),
            default={},
        )
        modifier = first_present(
            selected_entry.get("modifiedBy"),
            (selected_experiment or {}).get("modifiedBy"),
            default={},
        )
        primary_pubs = first_present(
            selected_entry.get("primaryPublicationIdentifiers"),
            (selected_experiment or {}).get("primaryPublicationIdentifiers"),
            default=[],
        )
        doi_ids = first_present(
            selected_entry.get("doiIdentifiers"),
