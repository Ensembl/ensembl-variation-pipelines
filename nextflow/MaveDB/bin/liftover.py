...
def log(reason, subid="na", **kv):
    ...
    sys.stderr.write(
        f"[{ts}][MaveDB][URN={URN}][STEP={STEP}][REASON={reason}][SUBID={subid}] {extras}\n"
    )
    sys.stderr.flush()
...
def main():
    ...
    parser.add_argument(
        "--metadata", type=str, help="path to file with MaveDB metadata"
    )
    parser.add_argument(
        "--mapped_variants",
        type=str,
    ...
    parser.add_argument(
        "--reference", type=str, default="hg38", help="genome (default: 'hg38')"
    )
...
def liftover_variants(mapped, genome, reference):
    ...
    with open(mapped) as f:
        ...
        for line in f:
            n_total += 1
            fields = line.rstrip("\n").split("\t")
            chr_, start, end = fields[0:3]

            # preferred SUBID: accession; else coord token
            subid = (
                fields[acc_idx]
                if acc_idx is not None and acc_idx < len(fields)
                else f"{chr_}:{start}-{end}"
            )
            ...
            if len(conv) > 1:
                log(
                    "liftover_multiple_mappings_start",
                    subid=subid,
                    chr=chr_,
                    pos=start,
                    n=len(conv),
            ...
            if len(conv) > 1:
                log(
                    "liftover_multiple_mappings_end",
                    subid=subid,
                    chr=chr_,
                    pos=end,
                    n=len(conv),
            ...
            # write lifted row
            fields[0:3] = new_chr.replace("chr", ""), str(new_start), str(new_end)
