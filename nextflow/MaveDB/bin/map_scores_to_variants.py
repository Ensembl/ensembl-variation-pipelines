...
def main():
    ...
    map = map_scores_to_variants(
        scores, mappings, metadata, mavedb_ids, hgvsp2vars, args.round
    )
...
def map_scores_to_variants(
    scores, mappings, metadata, map_ids, matches=None, round=None
):
    ...
    for row in scores:
        ...
        if "mavedb_id" in mapping.keys() and row["accession"] != mapping["mavedb_id"]:
            warnings.warn(
                "URN mismatch: trying to match "
                + row["accession"]
                + " from scores file with "
                + mapping["mavedb_id"]
                + " from mappings file"
            )
            continue
