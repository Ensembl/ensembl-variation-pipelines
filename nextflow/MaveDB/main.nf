...
if (params.help) {
  ...
  Usage:
    nextflow run nextflow/MaveDB/main.nf --urn urns.txt --from_files true \\
      --scores_path data/scores --mappings_path data/mappings --metadata_file data/main.json

  Mandatory arguments:
    --urn           File with one MaveDB URN per line
  ...
  Optional arguments:
    ...
    --from_files    Use local files instead of downloading via the MaveDB API (default: true)
    ...
    --metadata_file Path to MaveDB metadata file (combined metadata JSON)
...
include { merge_previous_output; reuse_previous_output } from './nf_modules/merge_previous_output.nf'
include { build_run_plan } from './nf_modules/planning.nf'
...
workflow {
  ...
  if (plan.reuse_only) {
    ...
    tabix_input = reuse_previous_output(file(plan.previous_output), plan.skipped_urns_file)
  } else {
    ...
    if (params.from_files) {
      ...
      // Log URNs excluded by the licence filter.
      metaChannel
      metaChannel
    ...
    tabix_input = concatenate_files.out
  ...
  log_collation_input = tabix_out.data.map { t -> [workflow.workDir.toString(), t] }
  collate_logs(log_collation_input)

  // datacheck: compare final URNs against input list and pull logs for any missing URNs
  datacheck_input = tabix_out.data.combine(collate_logs.out).map { output, logs -> tuple(output, plan.urn_file_for_datacheck, logs) }
