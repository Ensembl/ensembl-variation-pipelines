...
# Use numeric flags for Perl boolean checks.
die "offline must be 0 or 1\n" unless defined($offline) && $offline =~ /\A[01]\z/;
...
if ( $any_results ){
  ...
  if ($sqlite){
    ...
    $dbh->do("CREATE TABLE IF NOT EXISTS predictions (md5 TEXT NOT NULL, analysis INTEGER NOT NULL, matrix BLOB NOT NULL, PRIMARY KEY (md5, analysis))");
    my $sth = $dbh->prepare("INSERT OR REPLACE INTO predictions (md5, analysis, matrix) VALUES(?, ?, ?)");

    my $attrib_id = $model_name eq "humdiv" ? 269 : 268;
    $sth->bind_param(1, $pred_matrix->translation_md5);
    $sth->bind_param(2, $attrib_id);
    $sth->bind_param(3, $pred_matrix->serialize, SQL_BLOB);
    $sth->execute;
    $dbh->disconnect;
  }
} else {
