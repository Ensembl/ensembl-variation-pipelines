...
// Merge databases after all writers finish.
process postprocess_sqlite_db {
  container "ensemblorg/ensembl-vep:latest"
  publishDir file(params.sqlite_db).parent.toString(), mode: 'copy', overwrite: true
  cache false

  input:
    path databases, stageAs: 'shards/part??/*'

  output:
    path "${file(params.sqlite_db).name}"

  script:
  def output = "'" + file(params.sqlite_db).name.replace("'", "'\"'\"'") + "'"
  """
  merge_prediction_databases.pl ${output}
  """
}
