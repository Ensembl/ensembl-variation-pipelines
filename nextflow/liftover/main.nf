...
if (params.help) {
  ...
  Mandatory arguments:
    ...
    --ids        File containing exactly one target genome ID
    --chain      Path to a chain file for the single target in --ids
    --src_fasta  Path to source fasta file
    --dst_fasta  Path to destination fasta file 

  Optional arguments:
    --out_dir    Path to output directory (default: 'output')
    --report     Filename with remapping stats (default: 'bcftools-liftover_report.txt')
    --keep_id    Keep the supplied ID in output filenames (default: false;
                 use the assembly lookup where available)
  """
  exit 1
}
...
// Stage reference indexes alongside their FASTAs, including compressed FASTA indexes.
def reference_files(value) {
  def fasta = file(value, checkIfExists: true)
  def files = [fasta, file(value + '.fai', checkIfExists: true)]
  if (fasta.name ==~ /.*\.(gz|bgz|bgzf)$/) {
    files << file(value + '.gzi', checkIfExists: true)
  }
  return files
}

workflow {
  for (name in ['ids', 'vcf', 'chain', 'src_fasta', 'dst_fasta']) {
    if (!params[name]) exit 1, "ERROR: --${name} is required"
  }
  def ids = file(params.ids, checkIfExists: true).readLines().collect { it.trim() }.findAll { it }
  if (ids.size() != 1) {
    exit 1, "ERROR: --ids must contain exactly one target ID for the supplied chain and destination FASTA"
  }
  if (!(ids[0] ==~ /[A-Za-z0-9_.-]+/)) {
    exit 1, "ERROR: target ID may contain only letters, digits, underscores, dots and hyphens"
  }
  vcf = Channel.fromPath(params.vcf, checkIfExists: true)
  def chain = file(params.chain, checkIfExists: true)
  def source = reference_files(params.src_fasta)
  def destination = reference_files(params.dst_fasta)

  bcftools_liftover(vcf, ids[0], chain, source, destination)
