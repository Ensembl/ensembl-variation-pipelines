...
all_annot_pd = pd.read_csv(
    args.gff3, delimiter="\t", comment="#", header=None, names=colnames, dtype=str
)

# Identify genes by GFF3 ID, regardless of feature type.
...
genes_pd = genes_pd.assign(
    ...
    gene_symbol=genes_pd["attribute"].str.extract(r"(?:^|;)Name=([^;]+)", expand=False),
)

if args.go:
    # Identify transcripts by parent gene and transcript ID.
    ...
    annot_pd = annot_pd.assign(
        gene=parent_genes[transcript_mask], transcript=transcript_ids[transcript_mask]
    )
    annot_pd = pd.merge(
        annot_pd, genes_pd[["gene", "gene_symbol"]], on="gene", how="inner"
    )
else:
    annot_pd = genes_pd
...
ref_pd = pd.read_csv(
    annot, delimiter="\t", comment="#", header=None, names=colnames, dtype=str
)
...
if args.gene_symbols is not None:
    ...
    symbols = pd.read_csv(
        args.gene_symbols, delimiter="\t", names=["gene_symbol", "gene"]
    )
    ref_pd = pd.merge(ref_pd, symbols, on="gene", how="inner")

# join annotations based on gene symbols
print("Joining annotation...", flush=True)
joint = pd.merge(annot_pd, ref_pd, on="gene_symbol", how="inner")

if args.go:
    # Keep GO annotations transcript-based for plugin compatibility.
    joint = joint.assign(
        go_terms=joint["attribute_y"].str.extract(r";(Ontology_term=.*)")
    )
    joint = joint.assign(
        new_attribute="ID=" + joint["transcript"] + ";" + joint["go_terms"]
    )
elif args.pheno:
    # prepare new assembly-specific Phenotypes annotations
    joint = joint.assign(
        phenotype=joint["attribute_y"].str.extract(r"; (phenotype=.*)")
    )
    joint = joint.assign(
        new_attribute="id=" + joint["gene_x"] + "; " + joint["phenotype"]
    )
...
if len(new_gtf) == 0:
    raise Exception(
        f"ERROR: new pangenomes {plugin} annotation is empty (maybe no genes matched between annotations?)"
    )
