...
A detailed nf-core-style pipeline diagram is available at [`pipeline_diagram.drawio`](pipeline_diagram.drawio).

Open it with one of:
- **draw.io web app** — drag and drop the file into [app.diagrams.net](https://app.diagrams.net)
- **VS Code** — install the [Draw.io Integration](https://marketplace.visualstudio.com/items?itemName=hediet.vscode-drawio) extension

The diagram shows all processes, subworkflows, conditional branches (`skip_vep`, `skip_stats`, `skip_tracks`), data flow, and output files with nf-core colour coding.

### Auto-generated DAG

The following DAG was generated by nextflow using the `with-dag` parameter.
