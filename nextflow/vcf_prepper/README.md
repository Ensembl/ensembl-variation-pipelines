...
- `plugins`: Object describing plugin configuration.
  ...
  - `confs`: array of plugin config objects, each with:
    ...
    - `files`: array of file objects (e.g., `{ snv: string }`)
    - `file_factory`: (string) factory to use in case we need to fetch and process plugin files
    - `params`: array of param objects (e.g., `{ key: string }`)
- `custom_annotaions`: Object describing custom annotation configuration.
  - `genome_matcher`: (string) the matcher class to use for matching the genome against avaialble custom annotations.
  - `confs`: array of custom annotation config objects, each with:
    - `name`: (string) name of the custom annotation
    - `genome_matcher`: (string) the matcher class to use for matching the genome against this custom annotation
    - `file_factory`: (string) factory to use in case we need to fetch and process custom annotation files
    - `params`: array of param objects (e.g., `{ short_name: string, file: string, format: string, type: string, coord: integer }`)
