...
def main(args=None):
    ...
    custom_annotations_config_file = genome_uuid + ".plugin.txt"
    ...
    with open(custom_annotations_config_file, "w") as f:
        ...
        for custom_annotation_config in config_json:
            annotation_type = custom_annotation_config["name"]
            
            module = importlib.import_module("custom_annotation")
            ...
            custom_annotation_builder.version = version

            if not custom_annotation_builder.match(annotation_type):
                continue

            custom_annotation_args = custom_annotation_builder.get_args(annotation_type)

            for arg in custom_annotation_config["args"]:
                custom_annotation_args[arg] = custom_annotation_args["args"].get(arg)

            f.write("custom " 
                    + ",".join([f"{k}={v}" for k, v in custom_annotation_args.items()]) 
                    + "\n"
                )
