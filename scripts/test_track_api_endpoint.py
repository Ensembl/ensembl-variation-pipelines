...
def main(argv):
    ...
    if not os.path.isfile(track_metadata_file):
        raise FileNotFoundError(f"No such file - {track_metadata_file}, please provide correct track metadata file")
    ...
    with open(track_metadata_file, "r") as f:
        ...
        for genome_uuid in track_metadata:
            ...
            bigbed = os.path.basename(track_metadata[genome_uuid]["datafiles"]["details"])
            bigwig = os.path.basename(track_metadata[genome_uuid]["datafiles"]["summary"])
            ...
            if data:
                for category in data['track_categories']:
                    ...
                    if len(category["track_list"]) > 1:
                        print(f"EXPECTED: single tracks, but {len(category["track_list"])} tracks found")
